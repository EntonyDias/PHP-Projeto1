<!DOCTYPE html>
<html lang="pt-br">

<head>
   <meta charset="UTF-8">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title>Pagina de MiniProjetos</title>
   <link rel="stylesheet" href="./style.css">
</head>

<body>
   <header>
      <h3>Projetos</h3>
      <h3>Exercicios PHP</h3>
      <h3>Desde 2024/11/04 até 2024/11/??</h3>
   </header>
   <main>
      <div class="container">
         <!--Lista de Atividades-->
         <ul id="listaAtividades">
            <li>
               <p>Atividade</p>
               <img src="" alt="Pagina da 1ª Atividade">
               <p>Criação de formulário HTML para PHP</p>
               <a href="./Atividades/Atividade1/Atividade0.html">Visitar</a>
            </li>
            <li>
               <p>Atividade</p>
               <img src="" alt="Pagina da 2ª Atividade">
               <p></p>
               <a href="./Atividades/Atividade2/Atividade2.php">Visitar</a>
            </li>
            <li>
               <p>Atividade</p>
               <img src="" alt="Pagina da 3ª Atividade">
               <p></p>
               <a href="./Atividades/Atividade3/index.php">Visitar</a>
            </li>
         </ul>
         <!--Lista de Exercicio-->
         <ul id="listaExercicio">
            <li>
               <p>Exercicio</p>
               <img src="./Exercicios/images/Exercicio1.png" alt="Pagina do 1º Exercicio">
               <p>Aprendendo sobre os Forms</p>
               <a href="./Exercicios/Exercicio1/Exercicio1.php">Visitar</a>
            </li>
            <li>
               <p>Exercicio</p>
               <img src="" alt="Pagina do 2º Exercicio">
               <p></p>
               <a href="./Exercicios/Exercicio2/Exercicio2.php">Visitar</a>
            </li>
            <li>
               <p>Exercicio</p>
               <img src="" alt="Pagina do 3º Exercicio">
               <p></p>
               <a href="./Exercicios/Exercicio3/Exercicio3.php">Visitar</a>
            </li>
            <li>
               <p>Exercicio</p>
               <img src="" alt="Pagina do 3º Exercicio">
               <p></p>
               <a href="./Exercicios/Exercicio4/Exercicio4.php">Visitar</a>
            </li>
         </ul>
         
         <!--Lista de Treinos-->
         <ul id="listaTreinos">
            <li>
               <p>Treino</p>
               <img src="./Treinos/images/treino1.png" alt="Pagina do 1º Treino">
               <p>Básico do básico de PHP</p>
               <a href="./Treinos/Treino1.php">Visitar</a>
            </li>
            <li>
               <p>Treino</p>
               <img src="" alt="Pagina do 2º Treino">
               <p></p>
               <a href="./Treinos/Treino2.php">Visitar</a>
            </li>
            <li>
               <p>Treino</p>
               <img src="" alt="Pagina do 3º Treino">
               <p></p>
               <a href="">Visitar</a>
            </li>
         </ul>
      </div>
   </main>
   <footer>

   </footer>
</body>

</html>